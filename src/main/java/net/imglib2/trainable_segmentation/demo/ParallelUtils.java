...
public class ParallelUtils
{
	...
	public static <T> void chunkAndRunOperation(
			RandomAccessibleInterval<T> image,
			int[] cellDimensions,
			Consumer<RandomAccessibleInterval<T>> operation)
	{
		List< Callable< Void > > chunks = chunkOperation( image, cellDimensions, operation );
		List< Callable< Void > > chunksDisplayingProgress = ParallelUtils.addShowProgress( chunks );
		ParallelUtils.executeInParallel( Executors.newFixedThreadPool( Runtime.getRuntime().availableProcessors() ), chunksDisplayingProgress );
	}

	public static <T> List<Callable<Void>> chunkOperation(RandomAccessibleInterval< T > image, int[] cellDimensions, Consumer<RandomAccessibleInterval<T>> operation) {
	...
	public static Stream<Interval> getCells(CellGrid cellGrid) {
